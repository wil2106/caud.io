...
it('GET /api/sample/:id', async (done) => {
...
it('POST /api/sample/', async (done) => {
  await request
    .post(`/api/sample`)
    .send({title: "addSample", file:"addFile" })
    .set('Authorization', `bearer ${token}`)

  const data = await Client.query(`SELECT * FROM public.samples WHERE title='addSample';`)
  if(data.rowCount === 1)
      return done()
})